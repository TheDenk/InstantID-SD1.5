...
instantid_styles = {k["name"]: (k["prompt"], k["negative_prompt"]) for k in style_list}


def update_prompt_for_style(prompt, n_prompt, style):
    prompt_template, n_prompt_template = instantid_styles.get(style, ("{prompt}", ""))
    return (prompt_template.replace("{prompt}", prompt), n_prompt_template + n_prompt)