...
if hasattr(F, "scaled_dot_product_attention"):  ## Torch 2. is available
    from instantid.ip_adapter.attention_processor import IPAttnProcessor2_0 as IPAttnProcessor
else:
...
class StableDiffusionInstantPipeline(StableDiffusionControlNetPipeline):
    ...
    def set_image_proj_model(self, image_proj_model, image_emb_dim=512):
        self.image_proj_model = image_proj_model
        self.image_proj_model_in_features = image_emb_dim

    def load_instantid_components(self, adapter_ckpt_path, image_proj_ckpt_path, controlnet_ckpt_path, image_emb_dim=512):
        self.image_proj_model_in_features = image_emb_dim
        unet = getattr(self, self.unet_name) if not hasattr(self, "unet") else self.unet

        _ = set_ip_adapter_processors(unet, num_tokens=16, scale=1.0, ignore_motion=True);
        ...
        image_proj_ckpt = torch.load(image_proj_ckpt_path, map_location='cpu')
        controlnet_ckpt = torch.load(controlnet_ckpt_path, map_location='cpu')
        ...
        print(f'[ADAPTER LAYERS] MISSING: {len(m)} | UNEXPECTED: {len(u)} |')
        
        m, u = self.image_proj_model.load_state_dict(image_proj_ckpt['state_dict'])
        print(f'[IMAGE PROJ MODEL] MISSING: {len(m)} | UNEXPECTED: {len(u)} |')
        
        m, u = self.controlnet.load_state_dict(controlnet_ckpt['state_dict'])
        print(f'[CONTROLNET MODEL] MISSING: {len(m)} | UNEXPECTED: {len(u)} |')
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # print(controlnet_conditioning_scale, type(controlnet_conditioning_scale), isinstance(controlnet_conditioning_scale, float))
        ...
        prompt_image_emb = self._encode_prompt_image_emb(ip_adapter_image_embeds, 
                                                         device,
                                                         num_images_per_prompt,
                                                         self.unet.dtype,
                                                         self.do_classifier_free_guidance)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if guess_mode and self.do_classifier_free_guidance:
                    ...
                    controlnet_prompt_embeds = prompt_image_emb.chunk(2)[1]
                else:
